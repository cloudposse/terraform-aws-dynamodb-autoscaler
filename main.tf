...
resource "aws_iam_role" "autoscaler" {
  ...
  name               = "${module.this.id}${module.this.delimiter}autoscaler"
...
resource "aws_iam_role_policy" "autoscaler" {
  count  = module.this.enabled ? 1 : 0
  name   = "${module.this.id}${module.this.delimiter}autoscaler${module.this.delimiter}dynamodb"
...
resource "aws_iam_role_policy" "autoscaler_cloudwatch" {
  count  = module.this.enabled ? 1 : 0
  name   = "${module.this.id}${module.this.delimiter}autoscaler${module.this.delimiter}cloudwatch"
  role   = join("", aws_iam_role.autoscaler.*.id)
